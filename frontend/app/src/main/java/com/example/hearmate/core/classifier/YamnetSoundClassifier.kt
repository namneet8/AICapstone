...
/**
 * Real sound classifier using YAMNet TensorFlow Lite model
 * YAMNet has 521 audio event classes from AudioSet ontology
 */
@Singleton
class YamnetSoundClassifier @Inject constructor(
    ...
    override suspend fun classifySound(audioData: FloatArray): SoundDetectionResult =
        withContext(Dispatchers.Default) {
            try {
                ...
                // Debug logging
                Log.d("YAMNet", "Audio size: ${audioData.size}, Sample rate expected: 16kHz")
                Log.d("YAMNet", "Audio range: ${audioData.minOrNull()} to ${audioData.maxOrNull()}")
                ...
                Log.d("YAMNet", "Results size: ${results.size}")
                ...
                Log.d("YAMNet", "Top: $labelName (${topCategory?.score})")
                ...
                Log.e("YAMNet", "Classification error: ${e.message}", e)
                e.printStackTrace()
    ...
    /**
     * Reads labels from YAMNet class map (assets/yamnet_class_map.csv)
     * Expected CSV format: index,mid,display_name
     */
    private fun loadLabels(): List<String> {
        return context.assets.open("yamnet_class_map.csv").bufferedReader().useLines { lines ->
